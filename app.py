...
def create_app(config_object=None):
    ...
    def security_headers(response):
        ...
        response.headers.setdefault("Content-Security-Policy", "default-src 'self'; connect-src 'self' https://cdn.socket.io ws: wss:; script-src 'self' https://cdn.socket.io; style-src 'self' 'unsafe-inline'; img-src 'self' data:;")
        return response
    ...
    def conversations_alias():
        from database.models import ContactRequest, User
        ...
        return jsonify(conversations=[{"id": u.id, "astra_id": u.astra_id,
            "username": u.username, "display_name": u.display_name, "avatar": u.avatar,
            "astra_id_display": f"ASTRA-{u.astra_id[:4]}-{u.astra_id[4:8]}-{u.astra_id[8:]}"} for u in users])

    @app.route("/api/conversations/<user_id>/messages", methods=["GET", "POST"])
    @require_auth
    def conversation_messages_alias(user_id):
        from backend.messaging import history, send
        return history(user_id) if request.method == "GET" else send(user_id)
    ...
    def tamper():
        ...
        for model in (User, ContactRequest, Message):
            for row in model.query.order_by(model.id).all():
                values = {c.name: str(getattr(row, c.name)) for c in model.__table__.columns}
                rows.append(model.__tablename__ + ":" + json.dumps(values, sort_keys=True, default=str))
        digest = hmac.new(master_key(app.config), "\n".join(rows).encode(), hashlib.sha256).hexdigest()
        expected = request.args.get("digest") or (request.get_json(silent=True) or {}).get("digest")
        return jsonify(algorithm="HMAC-SHA256", digest=digest, rows=len(rows),
                       valid=(hmac.compare_digest(digest, expected) if expected else None))
...
if __name__ == "__main__":
    app.extensions["socketio"].run(app, host="0.0.0.0", port=int(__import__("os").getenv("PORT", "5000")))
